...
void execute(BYTE opcode)
{
    ...
    // This part has been edited so that only the correct
    // Opcodes get sent to the Group_2_Move function
	if ((opcode >= 0xA0) && (opcode <= 0xA5) ||
		(opcode >= 0xB0) && (opcode <= 0xB5) ||
		(opcode >= 0xC0) && (opcode <= 0xC5) ||
		(opcode >= 0xD0) && (opcode <= 0xD5) ||
		(opcode >= 0xE0) && (opcode <= 0xE5) ||
...
void emulate()
{
    ...
    // Sanity has been added so that the marking server does not go into a loop.
	while ((!halt) && (memory_in_range) && sanity < 250) {
